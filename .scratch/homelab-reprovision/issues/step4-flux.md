...
**Command:** `ansible-playbook -e @vars.yaml -e @secrets.local.yaml playbooks/flux.yaml`
...
- [x] Add `github_owner`, `github_repo` to `vars.yaml` and `github_token` to `secrets.local.yaml`
